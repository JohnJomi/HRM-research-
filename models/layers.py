...
def _scaled_dot_product_attention(q, k, v, causal=False):
    """
    Fallback implementation of scaled dot product attention.
    Replaces FlashAttention which is not available on CPU/Apple Silicon.
...
class Attention(nn.Module):
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Scaled dot product attention (fallback from FlashAttention)
        attn_output = _scaled_dot_product_attention(q=query, k=key, v=value, causal=self.causal)
